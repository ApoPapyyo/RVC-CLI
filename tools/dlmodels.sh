...
check_file_pretrained() {
  printf "checking %s\n" "$2"
  if [ -f "./models/""$1""/""$2""" ]; then
      printf "%s in ./models/%s checked.\n" "$2" "$1" 
  else
      echo failed. starting download from huggingface.
      if command -v aria2c > /dev/null 2>&1; then
          aria2c --console-log-level=error -c -x 16 -s 16 -k 1M https://huggingface.co/lj1995/VoiceConversionWebUI/resolve/main/"$1"/"$2" -d ./models/"$1" -o "$2"
          [ -f "./models/""$1""/""$2""" ] && echo "download successful." || { echo "please try again!" && exit 1; }
      else
...
check_file_special() {
  ...
  if [ -f "./models/""$1""/""$2""" ]; then
      ...
      echo failed. starting download from huggingface.
      if command -v aria2c > /dev/null 2>&1; then
          aria2c --console-log-level=error -c -x 16 -s 16 -k 1M https://huggingface.co/lj1995/VoiceConversionWebUI/resolve/main/"$2" -d ./models/"$1" -o "$2"
          [ -f "./models/""$1""/""$2""" ] && echo "download successful." || { echo "please try again!" && exit 1; }
      else
          echo "aria2c command not found. Please install aria2c and try again."
          exit 1
