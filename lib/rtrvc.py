...
class RVC:
    def __init__(
        ...
        try:
            ...
            if last_rvc is None or last_rvc.pth_path != self.pth_path:
                ...
                self.samplerate = last_rvc.samplerate
    ...
    def infer(
        ...
        upp_res = int(np.floor(factor * self.samplerate // 100))
        if upp_res != self.samplerate // 100:
            if upp_res not in self.resample_kernel:
                self.resample_kernel[upp_res] = Resample(
                    orig_freq=upp_res,
                    new_freq=self.samplerate // 100,
                    dtype=torch.float32,
                ).to(self.device)
